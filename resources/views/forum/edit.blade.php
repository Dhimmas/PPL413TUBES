<x-app-layout>
    ...
    <x-slot name="header">
        {{--  Jika Anda menggunakan @include('layouts.navigation') di sini, pastikan tidak ada duplikasi --}}
        {{--  Jika layouts.navigation adalah bagian dari x-app-layout, ini mungkin tidak diperlukan --}}
    </x-slot>

    {{-- Main Content --}}
    <div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-75 z-40 px-4 py-8 overflow-y-auto">
        <div class="max-w-xl w-full bg-white/10 backdrop-blur-lg rounded-2xl p-8 md:p-10 shadow-2xl border border-white/20 transition-all duration-300 transform hover:scale-[1.01] relative z-50">
            ...
            <form action="{{ route('forum.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    <label for="forum_category_id" class="block text-sm font-semibold text-white mb-2">Kategori</label>
                    <select name="forum_category_id" id="forum_category_id"
                            class="w-full bg-white/5 text-white border border-white/20 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition duration-200">
                        <option value="" class="text-black">Pilih Kategori (Opsional)</option>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('forum_category_id', $post->forum_category_id) == $category->id ? 'selected' : '' }} class="text-black">
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- Gambar --}}
                <div class="mb-6">
                    <label for="image" class="block text-sm font-semibold text-white mb-2">Unggah Gambar Baru (Opsional)</label>
                ...
                {{-- Tampilkan Gambar Lama & Opsi Hapus --}}
                @if ($post->image)
                    <div class="mb-8">
                        ...
                        <div class="mt-3">
                            ...
                            <label for="remove_image" class="text-sm text-white/70 ml-2">Hapus Gambar Ini saat update</label>
                        </div>
                    </div>
                @endif
